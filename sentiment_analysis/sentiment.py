...
def predictor(sentiment,test,n_gram,tf_idf, test_size):
    ...
    if tf_idf == True:
        if n_gram == True:
            ext_tdidf = feature_extraction.text.TfidfVectorizer(ngram_range=(1,2),stop_words="english",
                                              max_features=5000)
        else:
            ext_tdidf = feature_extraction.text.TfidfVectorizer(ngram_range=(1,1),stop_words="english",
                                              max_features=5000)
        ...
        if n_gram == True:
            ext = feature_extraction.text.CountVectorizer(ngram_range=(1,2),stop_words="english", 
                                              max_features=5000)
        else:    
            ext = feature_extraction.text.CountVectorizer(ngram_range=(1,1),stop_words="english", 
                                              max_features=5000)
    ...
    sgd = linear_model.SGDClassifier(penalty="l1",loss="hinge", n_iter = 100, n_jobs=-1, shuffle=True, alpha=0.0001)#clf_opt.best_params_["alpha"])
